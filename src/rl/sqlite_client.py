...
class SQLiteClient:
    ...
    def delete_with_raw_sql(
        self,
        raw_sql: str,
        params: dict | None = None,
    ) -> None:
        """
        Execute a raw SQL delete or update query that does not return rows.

        Args:
        - raw_sql (str): The raw SQL query to execute (e.g., DELETE or UPDATE statements).
        - params (dict, optional): Parameters to bind to the raw SQL query.

        Raises:
        - Exception: Any exceptions encountered during execution will propagate.
        """
        with self.get_db() as session:
            # Execute raw SQL with optional parameters
            session.execute(raw_sql, params or {})
